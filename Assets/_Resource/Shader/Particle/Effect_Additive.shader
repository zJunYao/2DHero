...
Shader "T1/Particles/Effect_Additive" 
{
	Properties 
	{
...
//	        ZTest Off
			...
			VS_OUTPUT vert(VS_INPUT In)
			{
				...
				#ifdef _FLOW_ON
					float2 flowUV = In.uv.xy * _FlowTex_ST.xy + _FlowTex_ST.zw;
					Out.flowUV.xy=FlowRotate(flowUV, _FlowRotate/57.2957796);
			...
			float4 frag(VS_OUTPUT In) : COLOR 
			{
				...
				color = lerp(color, luminosity, _LuminosityAmount); 
    	            ...
    	            float temp_a = saturate(dissolve_r / _EdgeSoft);
					...
					*/
					...
					color.rgb *= lerp(dissolveValues.x,dissolveValues.y,_DissolveBlend);
				#endif
